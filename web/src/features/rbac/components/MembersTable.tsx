...
export function MembersTable({
  ...
  const columns: LangfuseColumnDef<MembersTableRow>[] = [
    ...
    },
    {
      accessorKey: "orgRole",
      id: "orgRole",
      header: t("members.organizationRole"),
      headerTooltip: {
        description: t("members.orgRoleTooltip"),
      ...
      ? [
          {
            ...
            id: "projectRole",
            header: t("members.projectRole"),
            headerTooltip: {
              description: t("members.projectRoleTooltip"),
              href: "https://langfuse.com/docs/administration/rbac",
            },
            cell: ({
              row,
              ...
              if (!projectRolesEntitlement)
                return t("members.notAvailableOnPlan");
  ...
  if (project ? !hasProjectViewAccess : !hasOrgViewAccess) {
    return (
      <Alert>
        <AlertTitle>{t("members.accessDenied")}</AlertTitle>
        <AlertDescription>
...
const OrgRoleDropdown = ({
  ...
  const mut = api.members.updateOrgMembership.useMutation({
    onSuccess: (data) => {
      ...
      if (data.userId === session.data?.user?.id) void session.update();
      showSuccessToast({
        title: t("members.roleSaved"),
        description: t("members.orgRoleUpdated"),
        duration: 2000,
      });
    },
  });
  ...
  return (
    <Select
      ...
      onValueChange={(value) => {
        if (
          userId !== session.data?.user?.id ||
          confirm(t("members.confirmOrgRoleChange"))
        ) {
          mut.mutate({
            orgId,
            orgMembershipId,
...
const ProjectRoleDropdown = ({
  ...
  const mut = api.members.updateProjectRole.useMutation({
    onSuccess: (data) => {
      ...
      showSuccessToast({
        title: t("members.roleSaved"),
        description: t("members.projectRoleUpdated"),
        duration: 2000,
      });
    },
  });
  ...
  return (
    <Select
      ...
      onValueChange={(value) => {
        if (
          userId !== session.data?.user?.id ||
          confirm(t("members.confirmProjectRoleChange"))
        ) {
          mut.mutate({
            orgId,
            orgMembershipId,
